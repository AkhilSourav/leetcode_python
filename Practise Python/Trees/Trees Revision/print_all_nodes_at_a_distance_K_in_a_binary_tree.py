...
def print_all_nodes_at_a_distance_K_from_a_given_target(self, target, k ):
    ...
    while(len(queue) > 0 and cur_level < k):
        size = len(queue)
        while(size > 0):
            cur = queue.pop(0)
...
print_all_nodes_at_a_distance_K_from_a_given_target(root, 6, 2)
