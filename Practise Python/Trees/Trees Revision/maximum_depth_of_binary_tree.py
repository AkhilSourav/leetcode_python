...
# Getting the maximum depth of the binary tree
# Formula: 1 + max(LeftSubtree Height, RightSubtree Height)
def maximum_depth_of_binary_tree(self) -> int:
    if self is None:
        return 0
    
    # Getting the height of the left subtree
    lh = maximum_depth_of_binary_tree(self.left)
    # Getting the height of the right subtree
    rh = maximum_depth_of_binary_tree(self.right)
    
    # Returning the maximum depth of the binary tree
    # 1 is added because we are counting the current node also
    return 1 + max(lh,rh)
...
# Printing the maximum depth of the binary tree
print(f"Maximum depth of the binary tree is: {maximum_depth_of_binary_tree(root)}")
