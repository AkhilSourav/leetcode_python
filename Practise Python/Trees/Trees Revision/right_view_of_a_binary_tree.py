...
def horizontal_order_traversal(self):
    ...
    h_level = 0
    # Append the root node to the queue {node, horizontal_level}
    queue.append([self,h_level])
...
def get_horizontal_order(self):
    answer = horizontal_order_traversal(self)
    print(answer)
    
    # my_dict is a dictionary with key as horizontal level and value as list of nodes at that level
    my_dict = {}
    for ans in answer:
    ...
    ans_list = []
    for key in my_dict:
        # RIGHT VIEW CONDITION OF A BINARY TREE #
...
print(f"Right View of Binary Tree: {get_horizontal_order(root)}")
